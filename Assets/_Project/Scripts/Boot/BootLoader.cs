...
public class BootLoader : MonoBehaviour
{
    ...
    private const string PREFIX = "BootLoader";
    #endregion
    ...
    private async UniTask InitializeAsync()
    {
        ...
        if (managersContainer != null)
        {
            ...
            GameLog.Log(PREFIX, "매니저 컨테이너 DontDestroyOnLoad 설정 완료");
        }
        ...
        GameLog.Log(PREFIX, $"총 {initializables.Length}개의 매니저 + 최소 로딩 시간 초기화 시작");

        // 모든 태스크 병렬 실행 (매니저 초기화 + 최소 로딩 시간)
        await UniTask.WhenAll(initTasks);

        GameLog.Log(PREFIX, "모든 매니저 초기화 완료");
    ...
    private async UniTask InitializeWithProgressAsync(IInitializable initializable, string managerName, CancellationToken cancellationToken)
    {
        try
        {
            ...
            GameLog.Log(PREFIX, $"{managerName} 초기화 시작");
            ...
            GameLog.Log(PREFIX, $"{managerName} 초기화 완료 ({completedCount}/{totalCount})");
        }
        catch (System.OperationCanceledException)
        {
            GameLog.Log(PREFIX, $"{managerName} 초기화 취소됨");
            throw;
        }
        catch (System.Exception ex)
        {
            GameLog.Error(PREFIX, $"{managerName} 초기화 실패: {ex.Message}");
            throw;
        }
    }
    ...
    {
        GameLog.Log(PREFIX, $"최소 로딩 시간 {minimumLoadingTime}초 대기 시작");
        ...
        GameLog.Log(PREFIX, $"최소 로딩 시간 완료 ({completedCount}/{totalCount})");
    }
