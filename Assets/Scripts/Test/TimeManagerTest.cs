    ...
    public class TimeManagerTest : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (pause != prevPause)
            {
                if (pause)
                {
                    TimeManager.Instance.Push(Defines.TimeId.Pause, 0f, 100);
                    GameLog.Log("TimeManagerTest", "일시정지 활성화");
                }
                else
                {
                    TimeManager.Instance.Remove(Defines.TimeId.Pause);
            ...
            if (slowMotion != prevSlowMotion)
            {
                if (slowMotion)
                {
                    TimeManager.Instance.Push(Defines.TimeId.SlowMotion, slowMotionScale, 10);
                    GameLog.Log("TimeManagerTest", $"슬로우모션 활성화 (배율: {slowMotionScale})");
                }
                else
                {
                    TimeManager.Instance.Remove(Defines.TimeId.SlowMotion);
            ...
            if (hitStop != prevHitStop)
            {
                if (hitStop)
                {
                    TimeManager.Instance.Push(Defines.TimeId.HitStop, 0f, 30);
                    GameLog.Log("TimeManagerTest", "히트스탑 활성화");
                }
                else
                {
                    TimeManager.Instance.Remove(Defines.TimeId.HitStop);
        ...
        {
            // 비활성화 시 모든 요청 제거
            if (pause) TimeManager.Instance.Remove(Defines.TimeId.Pause);
            if (slowMotion) TimeManager.Instance.Remove(Defines.TimeId.SlowMotion);
            if (hitStop) TimeManager.Instance.Remove(Defines.TimeId.HitStop);
        }
    }
}
